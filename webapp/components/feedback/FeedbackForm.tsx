...
export function FeedbackForm() {
  ...
  return (
    <>
        ...
        <div className="space-y-6">
          ...
          <div className="card-surface p-6">
            ...
            <p className="mt-3 text-sm leading-6 text-text-muted">
              Use the login page if you want to continue from an existing account.
            </p>
      ...
      {submitted && (
        <div className="fixed bottom-6 right-6 z-50 flex max-w-sm items-start gap-3 rounded-2xl border border-accent1/30 bg-surface px-5 py-4 shadow-[0_16px_40px_rgba(0,0,0,0.18)]">
          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-accent1/10 text-accent1">
            <Icon name="check" size={16} />
          </span>
          <div className="min-w-0 flex-1">
            <p className="text-sm font-semibold text-text">Thanks for your feedback</p>
            <p className="mt-0.5 text-xs text-text-muted">
              It has been captured for review.
            </p>
          </div>
          <button
            type="button"
            onClick={() => setSubmitted(false)}
            className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-text-muted hover:border-text/30 hover:text-text"
            aria-label="Close"
          >
